...
coco_class_labels = ('face', 'human')

coco_class_index = [0,1]


class COCODataset_FB_Diff_withSource(Dataset):
    """
    COCO dataset class.
    """
    def __init__(self, 
    ...
    def __getitem__(self, index):
        image, target, mask, source, source_mask = self.pull_item(index)
        return image, target, mask, source, source_mask
    ...
    def load_expanded_source_with_mask(self, img_id):
        image_info = self.coco.loadImgs(img_id)[0]
        source_id = image_info['source_id']
        source_path = os.path.join(self.source_folder, '{:012}'.format(source_id) + '.jpg')
        # print(f'souce_path: {source_path}')
        image = cv2.imread(source_path)
        source_height, source_width, _ = image.shape
        crop_xmin, crop_ymin, crop_width, crop_height = image_info['crop_info']
        
        expanded_xmin, expanded_ymin = crop_xmin - crop_width, crop_ymin - crop_height
        expanded_width, expanded_height = crop_width * 3, crop_height * 3
        
        expanded_xmax, expanded_ymax = expanded_xmin + expanded_width, expanded_ymin + expanded_height
        
        source_valid_xmin = max(0, expanded_xmin)
        source_valid_ymin = max(0, expanded_ymin)
        source_valid_xmax = min(source_width, expanded_xmax)
        source_valid_ymax = min(source_height, expanded_ymax)
        
        expanded_valid_xmin = max(0, -expanded_xmin)
        expanded_valid_ymin = max(0, -expanded_ymin)
        expanded_valid_xmax = min(expanded_width, source_width - expanded_xmin)
        expanded_valid_ymax = min(expanded_height, source_height - expanded_ymin)
        
        expanded_source_img = np.zeros((expanded_height, expanded_width, 3), dtype=np.uint8)
        expanded_source_mask = np.zeros((expanded_height, expanded_width), dtype=np.uint8)
        
        expanded_source_img[expanded_valid_ymin:expanded_valid_ymax, expanded_valid_xmin:expanded_valid_xmax] = \
            image[source_valid_ymin:source_valid_ymax, source_valid_xmin:source_valid_xmax]
            
        expanded_source_mask[expanded_valid_ymin:expanded_valid_ymax, expanded_valid_xmin:expanded_valid_xmax] = 1

        # print(f'expanded_source_mask.sum(): {expanded_source_mask.sum()}')

        return expanded_source_img, expanded_source_mask
    ...
    def pull_item(self, index):
        ...
        # load an image and target
            ...
            # print('image shape: ', image.shape, target)
            # augment
            image, target, mask = self.transform(image, target)
            
            source_img, source_mask = self.load_expanded_source_with_mask(img_id)
            
            source_img, _, source_mask = self.source_transform(source_img, None, source_mask)
            
        return image, target, mask, source_img, source_mask
...
if __name__ == "__main__":
    ...
    dataset = COCODataset_FB_Diff_withSource(img_size=max_size,
