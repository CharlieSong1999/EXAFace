...
class YOLOH(nn.Module):
    def __init__(self, 
        ...
        # expansion
        if 'expansion_cfg' in cfg.keys():
            self.expansion = get_Upsample_layer(**cfg['expansion_cfg'])
            if 'mlp_final' in cfg['expansion_cfg'] and cfg['expansion_cfg']['mlp_final']:
                self.mlpf = Conv(cfg['head_dim'], cfg['head_dim'], k=1, s=1, p=1)
            else:
                self.mlpf = None
        ...
        if 'residual_expansion' in cfg.keys():
            self.residual_expansion = cfg['residual_expansion']
        else:
            self.residual_expansion = False
    ...
    def inference_single_image(self, x):
        ...
        if hasattr(self, 'expansion') and self.expansion is not None:
            if self.residual_expansion:
                h_x, w_x = x.shape[2:]
                x = torch.nn.functional.pad(x, (h_x, h_x, w_x, w_x)) + self.expansion(x)
            else:
                x = self.expansion(x, trainable=False)
                
        if hasattr(self, 'mlpf') and self.mlpf is not None:
            x = self.mlpf(x)
    ...
    def heatmap(self, x):
        ...
        cls_pred = rearrange(cls_pred, 'b (h w k) c -> b (h w) c k', h=H, w=W, k=self.num_anchors).max(dim=-1)[0]
    ...
    def forward(self, x, mask=None):
        ...
        if not self.trainable:
            ...
            if hasattr(self, 'expansion') and self.expansion is not None:
                if self.residual_expansion:
                    h_x, w_x = x.shape[2:]
                    x = torch.nn.functional.pad(x, (h_x, h_x, w_x, w_x), mode='constant', value=0) + self.expansion(x)
                else:
                    x = self.expansion(x)
                    if torch.isnan(x).any():
                        print('Warning: expansion contains NaN')
                        raise ValueError('NaN in expansion')
                    
                    expanded_x = x

                if 'expansion' not in Has_print_dict:
                    print('expansion:', x.shape)
                    Has_print_dict['expansion'] = 1
            ...
            if 'head' not in Has_print_dict:
                print('head:', cls_pred.shape, reg_pred.shape)
